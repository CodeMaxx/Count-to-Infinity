...
function do_recv() {
    //window.alert("Message recvd1");
    console.log(">> do_recv");
    var msg = ws.rQshiftStr(ws.rQlen());
    ...
    while (ws.rQlen() > 1) {
        ...
        var recv_msg = ws.rQshiftStr((i-rQi) + 1);
...
function recvMsg(msg) {
    var messageVector = string2vector(msg);
    console.log(messageVector[0]);

    if(messageVector[0] === "/login"){ // login successful
        // go to chat page
       ...
       $('#registerform').hide();
    ...
    else if(messageVector[0] ===  "/wrong#"){ // wrong password
        ...
        for(i = 1; i < messageVector.length; i++){
            var username = messageVector[i];
            olusernames.push(username);
            i++;
            var name = messageVector[i];
            olnames.push(name);

            $('#collection_online').append( '<option value="1" id=' + username + '>' + username + '</option>');
        ...
        //"collection".append()
         ...
         for(i = 1; i < messageVector.length; i++){
            ...
            var online_user = messageVector[1];
            olusernames.push(online_user);
            $('#collection_online').append( '<option value="1" id=' + online_user + '>' + online_user + '</option>');
          
        }
        else if(messageVector[0] === "offline") {
           var offline_user  = messageVector[1];
           var index = olusernames.indexOf(offline_user);
            olusernames.splice(index, 1);

            for(i = 0; i < olusernames.length; i++){
            var username = olusernames[i];
            i++;
            var name = olusernames[i];
            $('#collection_online').append( '<option value="1" id=' + username + '>' + username + '</option>');
            }
        }
            
        else if(messageVector[0] === "message") {
            var from = messageVector[1];
            var msg = messageVector[2];

            $('#messages').append(from + ":" + msg + '<br>');
...
function string2vector(msg) {   // converts string of our protocol to a vector of strings
